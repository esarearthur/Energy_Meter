...
#define __GSM_NL "\r\n"
static const char __GSM_OK[] TINY_GSM_PROGMEM = "OK" __GSM_NL;
static const char __GSM_ERROR[] TINY_GSM_PROGMEM = "ERROR" __GSM_NL;
...
class TinyGsmSMS {
 public:
  ...
  int getUnreadSMSLocs(int *buf){
      return thisModem().getUnreadSMSLocsImpl(buf);
  }

  SMSmessage readSMS(uint8_t i){
      return thisModem().readSMSImpl(i);
  }
 ...
 protected:
  ...
  bool sendSMS_UTF16(const String &number, const void *text, size_t len) {
		...
		thisModem().waitResponse();
		thisModem().sendAT(GF("+CSCS=\"HEX\""));
		thisModem().thisModem().waitResponse();
		thisModem().sendAT(GF("+CSMP=17,167,0,8"));
		thisModem().waitResponse();

		thisModem().sendAT(GF("+CMGS=\""), number, GF("\""));
		if (thisModem().waitResponse(GF(">")) != 1) {
			return false;
		}
		...
		thisModem().stream.write((char)0x1A);
		thisModem().stream.flush();
		return thisModem().waitResponse(60000L) == 1;
	}

	int getUnreadSMSLocsImpl(int *buf) {
		String data;
		...
		thisModem().sendAT(GF("+CMGL=\"REC UNREAD\""));
		if(thisModem().waitResponse(5000L, GF("+CMGL:"), GFP(__GSM_OK), GFP(__GSM_ERROR))) {
			do {
				data = thisModem().stream.readStringUntil('\n');
				int pos = data.indexOf("REC UNREAD");
				if (pos > 0) {
					index = data.substring(0, pos - 1).toInt();
					msgcount[occurrences] = index;
					buf[occurrences] = index;
					Serial.println(buf[occurrences]);
					occurrences++;
				}
			} while (thisModem().waitResponse() == 1);
		}

		return occurrences;
	}

	SMSmessage readSMSImpl(uint8_t i) {
		char message[160];
		char number[50];
		char date[50];
		char type[10];
		...
		thisModem().sendAT(GF("+CMGR="), i);

		if (thisModem().waitResponse(10000L, GF("\r\n+CMGR:"))) {

			head = thisModem().stream.readStringUntil('\n');
			body = thisModem().stream.readStringUntil('\n');
